  ...
  render() {
    return (
      <tr onClick={() => this.setState({showDescription: !this.state.showDescription})}>
        <td className="fish-name">{this.state.showDescription}</td>
        <td>
          <img src={`http://tinyurl.com/h8o5szh`} />
        </td>
        {this.state.showDescription ? <td className="fish-description">Does anyone know where my dad is ${fishes.id.fishId} ?</td> : null}
      </tr>
    )
  }
}
